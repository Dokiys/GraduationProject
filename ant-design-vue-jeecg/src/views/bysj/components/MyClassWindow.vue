...
<script>
  ...
  export default {
    ...
    data () {
      return {
        that:this,
        checkedKeys:[], // 存储选中的班级id
        userId:"", // 存储用户id
        model:{}, // 存储SysUserDepartsVO表
        userDepartModel:{userId:'',departIdList:[]}, // 存储用户id一对多班级信息的对象
        departList:[], // 存储班级信息
        leafList:[],
        modalWidth:400,
        departTree:[],
    ...
    methods: {
      ...
      onCheck(checkedKeys, info){
        ...
        for (let i = 0; i < checkedNodes.length; i++) {
          ...
          if(!!!de.dataRef.children || de.dataRef.children.length == 0) this.departList.push(depart);
        }
      },
      queryDepartTree(){
        queryMyIdTree().then((res)=>{
            if(res.success){
                ...
                this.getClassIdsByRec(this.departTree[0]);
            }
        })
      },
      getClassIdsByRec(node){
        if(!!node.children && node.children.length > 0){
          node.children.forEach(item => {
            this.getClassIdsByRec(item);
          })
        }else{
          if(!!node && node.orgType==3) this.leafList.push(node.id);
        }
      }
    }
  }
