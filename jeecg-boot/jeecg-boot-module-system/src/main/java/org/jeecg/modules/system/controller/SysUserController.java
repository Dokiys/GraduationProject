...
public class SysUserController {
	...
	public Result<IPage<SysUser>> queryPageList(SysUser user,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        if(userIds!=null && userIds.size()>0){
            ...
            pageList.getRecords().forEach(item->{
                ...
                //获取角色名称
                List<SysUserRole> userRoles = sysUserRoleService.list(new QueryWrapper<SysUserRole>().lambda().eq(SysUserRole::getUserId, item.getId()));
                userRoles.forEach(userRole -> {
                    sysRoleList.forEach(sysRole -> {
                    if(sysRole.getId().equals(userRole.getRoleId())){
                        item.setRoleNames(sysRole.getRoleName() + ',');
                        }
                    });
                });
                //截取多角色格式
                if(item.getRoleNames()!=null && (item.getRoleNames().length()>1)){
                    int end = item.getRoleNames().length()-1;
                    String subRoleNames = item.getRoleNames().substring(0, end);
                    item.setRoleNames(subRoleNames);
                }
            });
    ...
    @RequestMapping(value = "/listWithoutDepart", method = RequestMethod.GET)
    public Result<IPage<SysUser>> queryPageListWithoutDepart(SysUser sysUser,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                                @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,HttpServletRequest req) {
        Result<IPage<SysUser>> result = new Result<IPage<SysUser>>();
        Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);
        ...
        IPage<SysUser> pageList = sysUserService.pageListWithoutDepart(page,username);
        //批量查询用户的所属部门
        //step.1 先拿到全部的 useids
        //step.2 通过 useids，一次性查询用户的所属部门名字
        List<String> userIds = pageList.getRecords().stream().map(SysUser::getId).collect(Collectors.toList());
    ...
    @RequestMapping(value = "/listOnlyTeacher", method = RequestMethod.GET)
    public Result<IPage<SysUser>> queryPageListOnlyTeacher(SysUser sysUser,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                                             @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,HttpServletRequest req) {
        Result<IPage<SysUser>> result = new Result<IPage<SysUser>>();
        Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);
        //获取当前用户id
        ...
        List<String> subDepIds = sysDepartService.getMySubDepIdsByDepId(departIds);
        ...
        IPage<SysUser> pageList = sysUserService.pageListOnlyTeacher(page,subDepIds,username);
        //批量查询用户的所属部门
        //step.1 先拿到全部的 useids
        //step.2 通过 useids，一次性查询用户的所属部门名字
        List<String> userIds = pageList.getRecords().stream().map(SysUser::getId).collect(Collectors.toList());
    ...
    public Result<IPage<SysUser>> departUserList(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        if("".equals(depId)){
            ...
            List<SysUserDepart> depIds = sysUserDepartService.list(queryWrapper);
            subDepids.add(depIds.get(0).getDepId());
        }else{
            subDepids.add(depId);
        }
        ...
        if(subDepids != null && subDepids.size()>0){
            ...
            pageList.getRecords().forEach(item -> {
                ...
                List<String> droleIds = sysDepartRoleUserService.getByUserId(item.getId());
                //获取部门角色名
                if(droleIds.size() > 0){
                    String departRoleName = sysDepartRoleService
                            .listByIds(droleIds)
                            .stream()
                            .map(SysDepartRole::getRoleName)
                            .collect(Collectors.joining(","));
                    item.setDepartRoleNames(departRoleName);
                }
            });
            result.setSuccess(true);
            result.setResult(pageList);
     ...
     * @return
     */
    @GetMapping("/verifyTeacherRoleByToken")
    public Result<?> verifyTeacherRoleByToken(HttpServletRequest request, @RequestParam(name = "token", required = false) String token) {
        try {
            String username = null;
            // 如果没有传递token，就从header中获取token并获取用户信息
            if (oConvertUtils.isEmpty(token)) {
            ...
            surQueryWrapper.eq("user_id",sysUser.getId());
            ...
            //如果角色为教师或者班主任则返回true
            if(collect.indexOf(BysjConst.SUR_MASTERT_CODE) != -1
                    || collect.indexOf(BysjConst.SUR_TEACHER_CODE) != -1) return Result.ok(true);
